...
def main():
    ...
    # Remove 'kayla1000/ArgoCD-GithubAction_Manifest_2Repo' directory if it exists
    if os.path.exists('ArgoCD-GithubAction_Manifest_2Repo'):
        shutil.rmtree('ArgoCD-GithubAction_Manifest_2Repo')

    # Update values.yaml in the kayla1000/ArgoCD-GithubAction_Manifest_2Repo
    update_values_yaml(value_to_update, property_path, github_token)

def update_values_yaml(new_value, property_path, github_token):
    # Clone the kayla1000/ArgoCD-GithubAction_Manifest_2Repo
    try:
        repository_url = f"https://{github_token}@github.com/kayla1000/ArgoCD-GithubAction_Manifest_2Repo.git"
        subprocess.run(['git', 'clone', '--depth', '1', repository_url, 'ArgoCD-GithubAction_Manifest_2Repo'])
        os.chdir('ArgoCD-GithubAction_Manifest_2Repo')
        ...
        subprocess.run(['git', 'config', 'user.email', 'orimoloyekayode@gmail.com'])
        subprocess.run(['git', 'config', 'user.name', 'kayla1000'])
